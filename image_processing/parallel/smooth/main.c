...
int main(int argc, char *argv[])
{
	...
	{	
		...
		for (i = 0; i < n_row; ++i)
		{
			MPI_Send(&padded_img_ptr->arr[(n_col + 2) * i], 3 * (n_col + 2), dt_pixel, receiver, 0, MPI_COMM_WORLD);
	...
	else
	{
		...
		Pixel *recv_pixel_arr = malloc(3 * (params[0] + 2) * sizeof(Pixel));
		Pixel *send_pixel_arr = malloc(params[0] * sizeof(Pixel));
		
		int i;
		for (i = 0; i < n_jobs; ++i)
		{
			MPI_Recv(recv_pixel_arr, 3 * (n_col + 2), dt_pixel,
				0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			...
			unsigned int idx = 0;
				...
				unsigned int avg_b_val = 0;
                        	...
                        	for (k = 0; k < 9; ++k)
                        	{
					unsigned int new_i = 1 + y_pos[k];
                               	 	unsigned int new_j = j + x_pos[k];

                          		avg_r_val += recv_pixel_arr[(n_col + 2) * new_i + new_j].r;
                               		avg_g_val += recv_pixel_arr[(n_col + 2) * new_i + new_j].g;
                                	avg_b_val += recv_pixel_arr[(n_col + 2) * new_i + new_j].b;
                        	}

                        	send_pixel_arr[idx].r = avg_r_val / 9;
                        	send_pixel_arr[idx].g = avg_g_val / 9;
                        	send_pixel_arr[idx].b = avg_b_val / 9;
